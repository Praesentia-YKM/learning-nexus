...
public class SubwayRecordDTO {
    ...
    private Map<String, Integer> passengerData = new HashMap<>();

    // JSON에서 `HR_`로 시작하는 데이터를 자동으로 passengerData에 저장
    @JsonAnySetter
    public void setDynamicField(String key, Object value) {
        if (key.startsWith("HR_") && value instanceof Number) {
            passengerData.put(key, ((Number) value).intValue());
        }
    }

    public void setHourData(String key, int value) {
        this.passengerData.put(key, value);
    }

    public int getInPassengers(int hour) {
        return passengerData.getOrDefault(String.format("HR_%d_GET_ON_NOPE", hour), 0);
    }

    public int getOutPassengers(int hour) {
        return passengerData.getOrDefault(String.format("HR_%d_GET_OFF_NOPE", hour), 0);
    }
